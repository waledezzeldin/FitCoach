...
exports.createIntakeRouter = void 0;
const express_1 = require("express");
const createIntakeRouter = (client) => {
    const router = (0, express_1.Router)();
    const serializeIntake = (record) => {
        if (!record) {
        ...
        const secondCompleted = record.age !== null &&
    ...
    router.post('/second', async (req, res) => {
        const { userId, age, weight, height, experienceLevel, workoutFrequency, injuries = [], completedAt, } = req.body;
        if (!userId) {
            return res.status(400).json({ error: 'userId is required' });
        }
        if (age === undefined ||
...
exports.createIntakeRouter = createIntakeRouter;
