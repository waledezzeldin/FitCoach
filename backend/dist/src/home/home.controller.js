...
function estimateCalories(workouts) {
...
function resolveTargetCalories(quota, preference) {
    if (preference?.dailyCalorieTarget && typeof preference.dailyCalorieTarget === 'number') {
        return preference.dailyCalorieTarget;
    }
    if (quota?.tier === client_1.SubscriptionTier.smart_premium) {
        return 2200;
    }
    if (quota?.tier === client_1.SubscriptionTier.premium) {
        return 2000;
    }
...
function computeWeeklyCompletion(workoutsCompleted) {
    if (!DEFAULT_WEEKLY_WORKOUTS)
        return 0;
    return Math.min(100, Math.round((workoutsCompleted / DEFAULT_WEEKLY_WORKOUTS) * 100));
}
function aggregateMacros(entries, targetCalories) {
...
function computeNutritionAdherence(entries, targetCalories) {
    if (!entries.length || !targetCalories)
        return 0;
...
function buildQuickActions(tier) {
    const isPremium = tier && tier !== client_1.SubscriptionTier.freemium;
    return {
...
function buildDemoSummary(persona) {
    ...
    return {
        ...
        quickActions: buildQuickActions(client_1.SubscriptionTier.smart_premium),
        quota: null,
    };
}
